...
fig, (ax0, ax1, ax2) = plt.subplots(3, 1, figsize=(8, 10), constrained_layout=True, sharex=True, gridspec_kw={'height_ratios': [1, 3, 3]})

# Plot 1D data in the top subplot
ax0.plot(sim.tiempo, Pot(AT[0]), label='Pot(AT[0])')
ax0.plot(sim.tiempo, Pot(AT[-1]), label='Pot(AT[-1])')
ax0.set_ylabel("Power (dB)", size=m_label_size)
ax0.legend(loc="upper right", prop={'size': 12})
ax0.tick_params(labelsize=tick_size)
ax0.set_xlim(Tlim)

# Imshow 1 in the middle subplot
im1 = ax1.imshow(P_T, cmap=cmap, aspect="auto", interpolation='bilinear', origin="lower",
                 extent=textent, vmin=vmin_t, vmax=vmax_t)
ax1.tick_params(labelsize=tick_size)
ax1.set_ylabel("Distance (m)", size=m_label_size)
...
# Compute the spectrogram in the bottom subplot
Pxx, freqs, bins, im2 = ax2.specgram(AT_spec, NFFT=700, noverlap=650, Fs=t_sampling, scale="dB", xextent=xextent, cmap=cmap)
...
# Create a single colorbar for both the middle and bottom subplots
cbar = fig.colorbar(im1, ax=[ax1, ax2], label='dB', location="top", aspect=30, pad=0.01)
cbar.set_label('dB', size=m_label_size, labelpad=10)
cbar.ax.tick_params(labelsize=cbar_tick_size)
...
# Plot 1D data in the top subplot
ax0.plot(sim.tiempo, Pot(AT[0])/np.max(Pot(AT[0])), label='Input', color="grey")
ax0.plot(sim.tiempo, Pot(AT[-1])/np.max(Pot(AT[0])), label='Output', color="#51127b", linewidth=2)
ax0.set_ylabel("Normalized\n Power", size=m_label_size)
ax0.legend(loc="upper right", prop={'size': 12})
ax0.tick_params(labelsize=tick_size)
ax0.set_xlim(Tlim)
...
# Add horizontal colorbar as an inset inside the middle subplot
# =============================================================================
# cax = fig.add_axes([0.7, 0.13, 0.25, 0.02])  # Adjust the position and size as needed
# cbar = fig.colorbar(im1, cax=cax, orientation='horizontal')
# cbar.set_label('dB', size=m_label_size, color='white')
# cbar.ax.tick_params(labelsize=cbar_tick_size, color='white', labelcolor='white')
# =============================================================================

# Add horizontal colorbar as an inset inside the middle subplot
cax = fig.add_axes([0.72, 0.08, 0.25, 0.02])  # Adjust the position and size as needed
cbar = fig.colorbar(im1, cax=cax, orientation='horizontal', location="top")
cbar.set_label('dB', size=m_label_size, color='white')
cbar.ax.tick_params(labelsize=cbar_tick_size, color='white', labelcolor='white', top=True, labeltop=True, bottom=False, labelbottom=False)



# Add a thin white outline to the colorbar using a patch
