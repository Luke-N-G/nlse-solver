...
from common.commonfunc import find_shift, find_chirp, Adapt_Vector, saver, loader, ReSim
...
darksol = simple_ds(sim.tiempo, fibra.beta2, fibra.gamma, width=ancho_1, maxwindow=Tmax)*0

#Calculamos el pulso inicial
signal = np.sqrt(amp_2)*np.exp( - ( (sim.tiempo - offset_2)/(np.sqrt(2)*ancho_2) )**2  )*np.exp(-2j*np.pi*dnu2*sim.tiempo)*1
...
plotcmap(sim, fibra, zlocs, A_t, A_w, Tlim=[-50,50], dB=False, wavelength=False, cmap="RdBu")
...
darksol_a = np.tanh(sim.tiempo) * SuperGauss(sim.tiempo,1,Tmax - 5,0,0,40)*1

#Calculamos el pulso inicial
signal_a = amp_2a * np.exp( -(sim_a.tiempo - offset_2a)**2/ancho_2a**2  ) * np.exp(-1j*omega_2a*sim.tiempo)*0
...
#---pcgNLSE---
t0 = time.time()
zlocs_a, A_wa, A_ta = SolveNLS(sim_a, fibra_a, pulso_a, z_locs=100)
#SolveNLS(sim, fibra, pulso, raman=False, z_locs=100)  #
t1 = time.time()
total_n = t1 - t0 #Implementar en Solve_pcGNLSE
print("Time",np.round(total_n/60,2),"(min)")
...
plotcmap(sim_a, fibra_a, zlocs_a, A_ta, A_wa, Tlim=[-60,60], dB=False, cmap="RdBu")
#plotinst(sim_a, fibra_a, A_ta, A_wa)
